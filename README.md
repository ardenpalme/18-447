...
to support the `addi` and `add` RISC-V instructions.  For Lab 1 Part B, you need to complete the single-cycle
...
This module is instantiate in the provided environment (using *named port* syntax) for simulation and synthesis.
...
execute `addi` and `add` instructions.
